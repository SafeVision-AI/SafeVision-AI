...
async def seed_blood_banks():
    ...
    out_path = Path(__file__).parents[1] / "datasets" / "emergency" / "blood_banks" / "india_blood_banks.json"
...
async def seed_police_stations():
    ...
    out_path = Path(__file__).parents[1] / "datasets" / "police" / "stations" / "india_police_stations.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump(all_items, f, indent=2)
...
async def seed_fire_stations():
    ...
    out_path = Path(__file__).parents[1] / "datasets" / "emergency" / "hospitals" / "india_fire_stations.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump(all_items, f, indent=2)
...
async def seed_hospitals():
    ...
    out_path = Path(__file__).parents[1] / "datasets" / "emergency" / "hospitals" / "india_hospitals_top25.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump(all_items, f, indent=2)
